...
class VoiceAssistant:
    def __init__(self, engine, agents, current_agent):
        self.engine = engine
        self.agents = agents
    ...
    def stop(self):
        ...
        # Matar procesos de espeak-ng pendientes
    ...
    def process_command(self, command_text):
        try:
            ...
            if output and output != "ERROR_SEGURIDAD_EGRESS":
                # Usar hilo para no bloquear mientras habla
                threading.Thread(target=self.tts.speak, args=(output, "es"), daemon=True).start()
            else:
                threading.Thread(target=self.tts.speak, args=("No pude procesar tu solicitud", "es"), daemon=True).start()
        except Exception as e:
            logger.error(f"Pipeline error: {e}")
            threading.Thread(target=self.tts.speak, args=("Error interno", "es"), daemon=True).start()

def init_voice_assistant(engine, agents, current_agent):
    assistant = VoiceAssistant(engine, agents, current_agent)
    assistant.start()
    return assistant
